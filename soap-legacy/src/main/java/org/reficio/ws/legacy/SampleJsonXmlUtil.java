...
class SampleJsonXmlUtil {
    ...
    public SampleJsonXmlUtil(boolean soapEnc, SoapContext context) {
    ...
    public String createSample(SchemaType sType,JSONObject typeElement) {
        ...
        createSampleForType(sType, cursor, typeElement);
     ...
     */
    public void createSampleForType(SchemaType stype, XmlCursor xmlc,JSON typeElement) {
        ...
        try {
            if (stype.isSimpleType() || stype.isURType()) {
                processSimpleType(stype, xmlc, typeElement);
...
//                    setJsonValue(typeElement,stype.getName().getLocalPart(),content);
                ...
                case SchemaType.MIXED_CONTENT:
                    ...
                    if (stype.getContentModel() != null) {
                        processParticle(stype.getContentModel(), xmlc, true,typeElement);
                    }
                    xmlc.insertChars(pick(WORDS));
                    break;
                case SchemaType.ELEMENT_CONTENT:
                    if (stype.getContentModel() != null) {
                        processParticle(stype.getContentModel(), xmlc, false,typeElement);
                    }
                    break;
            }
        } finally {
...
//        xmlc.insertChars(sample);
        String value = (String)((JSONObject)typeElement).get(stype.getName().getLocalPart());
...
//                return Double.valueOf(new Random().nextDouble()).toString();
     ...
     */
    private void processParticle(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement) {
        int loop = determineMinMaxForSample(sp, xmlc);

        while (loop-- > 0) {
            switch (sp.getParticleType()) {
                case (SchemaParticle.ELEMENT):
                    processElement(sp, xmlc, mixed, typeElement);
                    break;
                case (SchemaParticle.SEQUENCE):
                    processSequence(sp, xmlc, mixed,typeElement);
                    break;
                case (SchemaParticle.CHOICE):
                    processChoice(sp, xmlc, mixed,typeElement);
                    break;
                case (SchemaParticle.ALL):
                    processAll(sp, xmlc, mixed,typeElement);
                    break;
    ...
    private void processElement(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement) {
...
//            typeElement.put(element.getName().getLocalPart(), sp.getDefaultText());
            ...
            JSON subTypeElement = null;
            if (sp.getIntMaxOccurs() > 1) {
                JSONArray arr =  (JSONArray)((JSONObject)typeElement).get(element.getName().getLocalPart());
                for (Object jsonObj : arr) {
                    if(jsonObj instanceof JSON){
                        createSampleForType(element.getType(), xmlc, (JSON)jsonObj);
                    }else{
                        JSONObject tmpJson = new JSONObject(1);
                        tmpJson.put("string",jsonObj);
                        createSampleForType(element.getType(), xmlc, (JSON)tmpJson);
                    }

                }
            }else{
                Object elementObject = ((JSONObject) typeElement).get(element.getName().getLocalPart());
                if(elementObject instanceof JSON){
                    subTypeElement = (JSON)elementObject;
                }else{
                    JSONObject jsonObject = new JSONObject(1);
                    jsonObject.put("string",((JSONObject) typeElement).get(element.getName().getLocalPart()));
                    subTypeElement = jsonObject;
                }
                createSampleForType(element.getType(), xmlc, subTypeElement);
            }
...
//                setJsonValue(typeElement,element.getName().getLocalPart(),((JSONObject)subTypeElement).get("string"));
//            }else{
...
//            }
    ...
    private void processSequence(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement) {
...
//            if(sp1.getType().isSimpleType()){
//                String childElement = (String)((JSONObject) typeElement).get(sp1.getName().getLocalPart());
//                xmlc.insertElement(sp1.getName().getLocalPart(),childElement);
//                xmlc.toNextToken();
//            }else {
//                JSONObject childElement = (JSONObject)((JSONObject) typeElement).get(sp1.getName().getLocalPart());
            if (typeElement instanceof JSONArray) {
                typeElement = (JSON) ((JSONArray) typeElement).get(i);
            }
                processParticle(sp1, xmlc, mixed, typeElement);
//            }
    ...
    private void processChoice(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement) {
        SchemaParticle[] spc = sp.getParticleChildren();
        if (!skipComments)
            xmlc.insertComment("You have a CHOICE of the next " + String.valueOf(spc.length) + " items at this level");

        for (int i = 0; i < spc.length; i++) {
            processParticle(spc[i], xmlc, mixed,typeElement);
        }
    }

    private void processAll(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement) {
        SchemaParticle[] spc = sp.getParticleChildren();
        if (!skipComments)
