...
class SampleJson2Util {
    ...
    public SampleJson2Util(boolean soapEnc, SoapContext context) {
    ...
    public String createSample(SchemaType sType,JSONObject typeElement,Element node) {
        ...
        createSampleForType(sType, cursor, typeElement,node);
     ...
     */
    public void createSampleForType(SchemaType stype, XmlCursor xmlc,JSON typeElement,Element node) {
        ...
        try {
            if (stype.isSimpleType() || stype.isURType()) {
                String content = processSimpleType(stype, xmlc, typeElement);
                setJsonValue(typeElement,stype.getName().getLocalPart(),node.getStringValue());
                return;
            }
            ...
            processAttributes(stype, xmlc,typeElement);
            ...
            switch (stype.getContentType()) {
                ...
                case SchemaType.SIMPLE_CONTENT: {
                    String content = processSimpleType(stype, xmlc,typeElement);
                    setJsonValue(typeElement,stype.getName().getLocalPart(),content);
                }
                break;
                case SchemaType.MIXED_CONTENT:
                    xmlc.insertChars(pick(WORDS) + " ");
                    if (stype.getContentModel() != null) {
                        processParticle(stype.getContentModel(), xmlc, true,typeElement,node);
                    }
                    xmlc.insertChars(pick(WORDS));
                    break;
                case SchemaType.ELEMENT_CONTENT:
                    if (stype.getContentModel() != null) {
                        ...
                        if(name != null){
                             element = node.element(SampleJson2Util.getDom4jQname(name));
                        }else{
                            element=node;
                        }
                        processParticle(contentModel, xmlc, false,typeElement,element);
                    }
                    break;
            }
        } finally {
    ...
    private String processSimpleType(SchemaType stype, XmlCursor xmlc,JSON typeElement) {
        if (soapEnc) {
            QName typeName = stype.getName();
            if (typeName != null) {
                xmlc.insertAttributeWithValue(XSI_TYPE, formatQName(xmlc, typeName));
                setJsonValue(typeElement,XSI_TYPE.getLocalPart(),formatQName(xmlc, typeName));
            }
        }
...
//                return Double.valueOf(new Random().nextDouble()).toString();
     ...
     */
    private void processParticle(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement,Element node) {
        int loop = determineMinMaxForSample(sp, xmlc);

        while (loop-- > 0) {
            switch (sp.getParticleType()) {
                case (SchemaParticle.ELEMENT):
                    processElement(sp, xmlc, mixed, typeElement,node);
                    break;
                case (SchemaParticle.SEQUENCE):
                    processSequence(sp, xmlc, mixed,typeElement,node);
                    break;
                case (SchemaParticle.CHOICE):
                    processChoice(sp, xmlc, mixed,typeElement,node);
                    break;
                case (SchemaParticle.ALL):
                    processAll(sp, xmlc, mixed,typeElement,node);
                    break;
                case (SchemaParticle.WILDCARD):
                    processWildCard(sp, xmlc, mixed,typeElement);
                    break;
                default:
    ...
    private void processElement(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement,Element node) {
...
//            typeElement.put(element.getName().getLocalPart(),StringUtils.join(values, "s"));
            setJsonValue(typeElement,element.getName().getLocalPart(),StringUtils.join(values, "s"));
        }
        else if (sp.isDefault()) {
            xmlc.insertChars(sp.getDefaultText());
//            typeElement.put(element.getName().getLocalPart(), sp.getDefaultText());
            setJsonValue(typeElement,element.getName().getLocalPart(),sp.getDefaultText());
        }
        else {
            ...
            createSampleForType(element.getType(), xmlc, subTypeElement,node);

            QName qName = element.getType().getName();
            if(qName!=null && "string".equals(qName.getLocalPart())&& subTypeElement instanceof JSONObject){
                setJsonValue(typeElement,element.getName().getLocalPart(),node.getText());
            } else {
                setJsonValue(typeElement,element.getName().getLocalPart(),subTypeElement);
            }
        }
        // -> <elem>stuff</elem>^
        xmlc.toNextToken();
    ...
    private void processAttributes(SchemaType stype, XmlCursor xmlc, JSON typeElement) {
        if (soapEnc) {
            QName typeName = stype.getName();
            if (typeName != null) {
                xmlc.insertAttributeWithValue(XSI_TYPE, formatQName(xmlc, typeName));
                setJsonValue(typeElement,stype.getName().getLocalPart(),"application/?");
            }
        }
        ...
        for (int i = 0; i < attrProps.length; i++) {
            ...
            if (attr.getName().equals(new QName("http://www.w3.org/2005/05/xmlmime", "contentType"))) {
                ...
                if(typeElement instanceof JSONObject){
                    ((JSONObject)typeElement).put(stype.getName().getLocalPart(),"application/?");
                }else{
                    ((JSONArray) typeElement).add("application/?");
                }
            ...
            if (soapEnc) {
                ...
                if (ENC_ARRAYTYPE.equals(attr.getName())) {
                    ...
                    setJsonValue(typeElement,attr.getName().getLocalPart(),arrayType.soap11DimensionString());
                    continue;
                }
            }
            ...
            setJsonValue(typeElement,attr.getName().getLocalPart(),value);
        }
    }

    private void processSequence(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement,Element node) {
//        JSONArray jsonArray = new JSONArray();
//        ((JSONObject)typeElement).put(sp.getName().getLocalPart(),jsonArray);
        // if unbounded element is JsonArray, JsonArray passed down will create like entry:[{key:x},{value:y}]
        // we expected data is : entry:[{key:x,value:y}]
        SchemaParticle[] spc = sp.getParticleChildren();
        if(typeElement instanceof JSONArray){
            JSONArray tmp = (JSONArray) typeElement;
            typeElement = new JSONObject();
            tmp.add(typeElement);
        }
...
//            ((JSONArray)typeElement).add(element);
            SchemaParticle sp1 = spc[i];
            SchemaParticle contentModel = sp1.getType().getContentModel();

//            if (contentModel != null && contentModel.getIntMaxOccurs() > 1) {
//                JSONArray jsonArray = new JSONArray();
//                ((JSONObject) typeElement).put(sp1.getName().getLocalPart(), jsonArray);
//                processParticle(sp1, xmlc, mixed, jsonArray);
//            }
//
//           else if (sp1.getIntMaxOccurs()>1) {
//                JSONArray jsonArray = new JSONArray();
//                ((JSONObject)typeElement).put(sp1.getName().getLocalPart(),jsonArray);
//                processParticle(sp1, xmlc, mixed, jsonArray);
//            }else{
//                JSONObject element = new JSONObject();
//                ((JSONObject)typeElement).put(sp1.getName().getLocalPart(),element);
                processParticle(sp1, xmlc, mixed, typeElement,elements.get(i));
//            }
    ...
    private void processChoice(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement,Element node) {
        SchemaParticle[] spc = sp.getParticleChildren();
        if (!skipComments)
            xmlc.insertComment("You have a CHOICE of the next " + String.valueOf(spc.length) + " items at this level");

        for (int i = 0; i < spc.length; i++) {
            processParticle(spc[i], xmlc, mixed,typeElement,node);
        }
    }

    private void processAll(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement,Element node) {
        SchemaParticle[] spc = sp.getParticleChildren();
        if (!skipComments)
    ...
    private void processWildCard(SchemaParticle sp, XmlCursor xmlc, boolean mixed,JSON typeElement) {
        if (!skipComments)
    ...
    public static org.dom4j.QName getDom4jQname(QName elementName) {
        return new org.dom4j.QName(elementName.getLocalPart(), Namespace.get(elementName.getPrefix(), elementName.getNamespaceURI()));
    }
}
